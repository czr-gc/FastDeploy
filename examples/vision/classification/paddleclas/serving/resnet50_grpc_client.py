...
def preprocess(img):
    ...
    scaled = (typed / 255 - np.array([0.485, 0.456, 0.406], dtype=npdtype))/np.array([0.229, 0.224, 0.225], dtype=npdtype)
...
if __name__ == "__main__":
    ...
    image_data = Image.open("ILSVRC2012_val_00000010.jpeg")
    # im = cv2.imread("ILSVRC2012_val_00000010.jpeg")
    # im = np.array([im, ])
    im = preprocess(image_data)
    for i in range(1):
        result = runner.Run([im, ])
        for name, values in result.items():
            print("output_name:", name)
            print("top1:", values.argmax())
